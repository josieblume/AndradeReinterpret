/*
CREATED: APRIL 27th
Here is an interactive version of my interpretation of Andrade's piece, where depending on the location
of your mouse creates a new version of the piece each frame.
...
void draw() {
  ...
  for (int i = 0; i <raynum; i++) {
    px0 = 0 + cos(radians(mouseX))*(mouseY);
    py0 = 0 + sin(radians(mouseY))*(mouseX);
  ...
  noFill();
  for (int i = 0; i <=raynum; i++) {
    px = 0 + cos(radians(mouseX))*(mouseY);
